...
## Подключение правил к отдельному проекту

Project `AGENTS.md` должен указывать реальный путь к WM и требовать до начала
работы прочитать WM `AGENTS.md`, `rule/ai/0 owner-decisions.md`,
`aidocs/README.md` и относящиеся к задаче файлы `rule/ai`.
Ссылки на shared `web` или фраза «проект на WM» не заменяют этот шаг.
В локальном `AGENTS.md` явно повторить:

- PHP 7.2 и short tags; пробелы вокруг `=` и `=>` в новом и изменяемом коде.
- Логотип всегда является ссылкой на титульную страницу соответствующего сайта
  с `cursor: pointer`; проверять переход с вложенного URL.

Остальные правила читаются из актуальных источников WM, а не из устаревающей
копии. Локальные инструкции описывают особенности проекта и не отменяют
решения владельца. Проверка результата описана в [workflow](../workflow.md).
