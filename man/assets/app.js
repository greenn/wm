(() => {
    ...
    async function init() {
        ...
        console.log('[WM Docs] manifest:request', { url: 'content.php?action=manifest' });

        try {
            const response = await fetchJson('content.php?action=manifest');
    ...
    async function navigate(id, options = {}) {
        ...
        try {
            const response = await fetchJson(`content.php?doc=${encodeURIComponent(id)}`, {
