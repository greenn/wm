...
_needinc('css/cbn.php');
_needinc('css/dec.php');

_needphp('css/pcss.class');
_needphp('pl', 'valArray');
...
class _css {
	...
	//получаем данные
	static function getSubData($cfg, $prop){
		$data = _prop($cfg, $prop);
		if (is_string($data)) {
			$path = $data;
			if (!is_file($path)) {
...
//\
	...
	*/
	...
	static function googleFontsLinks($ffsData = true){
		...
		//dx(_css('ffs'), $linkUrl, $familyStack, static::$db);
		if (!Online) return '';
		return  <<<html
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
...
#< site_css
	...
	static function dec($value, $decMode = 'T', $addValues = false){
		static $CSS_MQ_DEC_MODE = array(
			...
			//'T1' => array(true, _css::DEC_T1, _css::DEC_T2),
			'T2' => array(true, _css::DEC_T1B, _css::DEC_T2B),
			'S'  => array(true, _css::DEC_S1, _css::DEC_S2),
			//'S1' => array(true, _css::DEC_S1, _css::DEC_S2),
			'S2' => array(true, _css::DEC_S1B, _css::DEC_S2B),
			'R' => array(
				'0' => true, '1' => 33, '2' => 60,
				'1b' => 20, '1c' => 27, '2b' => 45,
				'3' => 65, '4' => 67,
...
function _fs($id, $prop){
	//return _css('fs_', $id, $prop);
	static $_fs = array(); if (!$_fs) $_fs = _css('fs_');
...
function _fs_vw($mqName, $id, $prop = 'fs'){
	//return _vw(_css('fs_', $id, $prop), _mq($mqName), 2);
	return _vw(_fs($id, $prop), _mq($mqName), 2);
}
	/*  eq:
			_vw(_css('fs_', 'hg', 0), _mq(1), 2)
