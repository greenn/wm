...
if(!function_exists('strcode2utf')){
	function strcode2utf($str,$lo=true) {
		//converts all the &#nnn; and &#xhhh; in a string to Unicode
		if ($lo) { $lo = 1; } else { $lo = 0; } $GLOBALS['_strcode2utf_lo'] = $lo;
		$str = preg_replace_callback ('/\&\#([0-9]+)\;/m', "strcode2utf_callback1",$str);
		$str = preg_replace_callback ('/\&\#x([0-9a-fA-F]+)\;/m', "strcode2utf_callback2",$str);
		return $str;
	}
	function strcode2utf_callback1($matches){
		return code2utf($matches[1], $GLOBALS['_strcode2utf_lo']);
	}
	function strcode2utf_callback2($matches){
		return codeHex2utf($matches[1], $GLOBALS['_strcode2utf_lo']);
	}
}
