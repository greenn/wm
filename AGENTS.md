...
- В новом и изменяемом PHP-коде ставь пробелы с обеих сторон `=` и `=>`:
  `$wmRoot = tam_find_wm_root();`, `array('uv' => false)`. Не форматируй
  несвязанный legacy-код массово.
...
- Логотип всегда ведёт на титульную страницу соответствующего сайта:
  семантическая ссылка с корректным `href` и `cursor: pointer`.

## Передача правил проекту WM

При создании или настройке отдельного проекта WM закрепи в его `AGENTS.md`
реальный путь к WM и обязательное чтение WM `AGENTS.md`,
`rule/ai/0 owner-decisions.md`, `aidocs/README.md` и тематических правил.
Подключение общего `web` само по себе инструкции Codex не подключает.
Повтори в локальном `AGENTS.md` правила PHP-стиля и перехода по логотипу;
при изменениях PHP/UI проверяй их перед завершением задачи.
